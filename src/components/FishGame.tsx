...
function mkFood(foods: Food[]) {
  ...
  if (side===0){x=30+Math.random()*(W-60);y=-16;vx=(Math.random()-.5)*1.2;vy=spd;}
  else if (side===1){x=W+16;y=20+Math.random()*(H-40);vx=-spd;vy=(Math.random()-.5)*1.2;}
  else if (side===2){x=30+Math.random()*(W-60);y=H+16;vx=(Math.random()-.5)*1.2;vy=-spd;}
...
// Хлопья корма — «снежинка» из лепестков
...
// Рыбка полностью на Canvas
function drawFish(ctx: CanvasRenderingContext2D, x:number, y:number, angle:number, wag:number, scale=1) {
  ...
  ctx.bezierCurveTo(-42,11+wag*15,-32,13+wag*10,-20,0);
  ctx.closePath();
  const tg=ctx.createLinearGradient(-42,0,-20,0);
  tg.addColorStop(0,'#0369a1'); tg.addColorStop(1,'#0ea5e9');
  ctx.fillStyle=tg; ctx.fill();

  // Тело
  ctx.beginPath(); ctx.ellipse(0,0,24,12,0,0,Math.PI*2);
  const bg=ctx.createLinearGradient(-24,-12,24,12);
  bg.addColorStop(0,'#38bdf8'); bg.addColorStop(0.4,'#0ea5e9'); bg.addColorStop(1,'#0369a1');
  ctx.fillStyle=bg; ctx.fill();

  // Брюхо (светлое)
  ctx.beginPath(); ctx.ellipse(2,4,18,7,0.15,0,Math.PI*2);
  ctx.fillStyle='rgba(255,255,255,0.18)'; ctx.fill();

  // Верхний плавник
  ctx.beginPath(); ctx.moveTo(-8,-12); ctx.bezierCurveTo(-4,-22,9,-21,14,-12); ctx.closePath();
  ctx.fillStyle='rgba(14,165,233,0.75)'; ctx.fill();
  ctx.beginPath(); ctx.moveTo(-8,-12); ctx.bezierCurveTo(-4,-22,9,-21,14,-12);
  ctx.strokeStyle='rgba(3,105,161,0.5)'; ctx.lineWidth=0.8; ctx.stroke();
  ...
  // Чешуя
  for(let i=0;i<4;i++){
    ctx.beginPath(); ctx.ellipse(-10+i*9, -1, 6, 4, -0.2, Math.PI, Math.PI*2);
    ctx.strokeStyle='rgba(255,255,255,0.18)'; ctx.lineWidth=0.9; ctx.stroke();
  }

  // Боковая полоса
  ctx.beginPath(); ctx.moveTo(-16,0); ctx.bezierCurveTo(-4,-5,8,-4,20,0);
  ...
  eg.addColorStop(0,'#1e3a5f'); eg.addColorStop(1,'#0c1f35');
  ctx.fillStyle=eg; ctx.fill();
  ctx.beginPath(); ctx.arc(17,-4,1.2,0,Math.PI*2); ctx.fillStyle='rgba(255,255,255,0.9)'; ctx.fill();

  // Рот
  ctx.beginPath(); ctx.arc(23,2,3.5,0.3,Math.PI*0.85);
  ctx.strokeStyle='rgba(3,105,161,0.8)'; ctx.lineWidth=1.5; ctx.stroke();

  ctx.restore();
}

function drawBg(ctx: CanvasRenderingContext2D, time:number, bubbles: {x:number;y:number;r:number;speed:number;op:number}[]) {
  const bg=ctx.createLinearGradient(0,0,0,H);
  bg.addColorStop(0,'#0c4a6e'); bg.addColorStop(1,'#082f49');
  ctx.fillStyle=bg; ctx.fillRect(0,0,W,H);
  // каустики
  ctx.globalAlpha=0.04; ctx.fillStyle='#7dd3fc';
  for(let i=0;i<5;i++){
    const cx=((i*173+time*0.007)%W), cy=((i*97+time*0.004)%H);
    ctx.beginPath(); ctx.ellipse(cx,cy,40+i*8,7,Math.sin(time*0.001+i)*0.5,0,Math.PI*2); ctx.fill();
  }
  ctx.globalAlpha=1;
  // пузырьки
  bubbles.forEach(b=>{
    b.y-=b.speed; if(b.y<-b.r)b.y=H+b.r;
    ...
    ctx.fillStyle=`rgba(255,255,255,${b.op*0.6})`; ctx.fill();
  });
}
...
export default function FishGame({tgChannel, scoreToWin=WIN, promoCode='AQUA10'}: Props) {
  ...
  const stateRef = useRef({
    ...
    bubbles:Array.from({length:14},()=>({x:Math.random()*W,y:Math.random()*H,r:1.5+Math.random()*3.5,speed:0.2+Math.random()*0.35,op:0.04+Math.random()*0.07})),
  });
  ...
  // Idle-анимация — рыбка плавает на фоне
  const idleLoop = useCallback((time:number) => {
    const canvas = canvasRef.current; if(!canvas) return;
    const ctx = canvas.getContext('2d')!;
    const s = stateRef.current;
    drawBg(ctx, time, s.bubbles);
    const fx = W/2 + Math.sin(time*0.0008)*80;
    const fy = H/2 + Math.sin(time*0.0005)*40;
    const angle = Math.sin(time*0.0008)*0.4;
    const wag = Math.sin(time*0.006)*0.8;
    drawFish(ctx, fx, fy, angle, wag, 1.4);
  ...
  const gameLoop = useCallback((time:number) => {
    ...
    s.foods=s.foods.filter(f=>{
      ...
      if(d<(f.type==='worm'?38:30)){ s.score+=f.type==='worm'?2:1; setScore(s.score); if(s.score>=scoreToWin){s.alive=false;setPhase('won');} return false; }
      return true;
    });
    ...
    ctx.fillText(`${s.score} / ${scoreToWin}`, 36, 26);
    // иконка хлопья рядом со счётом
    drawFlake(ctx, 20, 20, time*0.001);
  ...
  const startGame = () => {
    ...
    const s=stateRef.current;
    s.fishX=W/2;s.fishY=H/2;s.fishVx=0;s.fishVy=0;s.targetX=W/2;s.targetY=H/2;
    s.fishAngle=0;s.wag=0;s.foods=[];s.score=0;s.alive=true;s.foodTimer=0;s.lastTime=performance.now();
  ...
  return (
    <div className="relative w-full" style={{maxWidth:W}}>
      ...
      {/* Idle overlay */}
      {phase==='idle' && (
        <div className="absolute inset-0 flex flex-col items-end justify-end rounded-2xl p-6 pointer-events-none">
          <div className="pointer-events-auto w-full flex flex-col items-center">
            <h3 className="text-2xl font-bold text-white mb-1 drop-shadow">Накорми рыбку!</h3>
            <p className="text-white/70 text-sm mb-1">Веди мышью — рыбка плывёт следом</p>
            <p className="text-white/70 text-sm mb-4">Собери <strong className="text-cyan-300">{scoreToWin} очков</strong> — получи промокод</p>
